...
describe("remote audio Supabase migration", () => {
  ...
  it("defaults to RLS denial, grants RPCs only, and configures idempotent realtime plus retention", () => {
    ...
    expect(migration()).toMatch(
      /do \$\$[\s\S]*alter publication supabase_realtime add table public\.crew_sessions;[\s\S]*alter publication supabase_realtime add table public\.remote_commands;[\s\S]*duplicate_object/i,
    );
