...
import { submitAmResetRequest } from "@/lib/staff-password-reset.server";
import { getOwnerLoginClientKey } from "@/lib/owner-login-client-key";

export const Route = createFileRoute("/am/forgot")({
  head: () => ({
    meta: [{ title: "Lupa Password Area Manager - LIME" }, { name: "robots", content: "noindex" }],
  }),
  component: AmForgotPage,
});

function AmForgotPage() {
    ...
    return (
      <AuthLayout>
        ...
        <p className="text-sm text-ta-gray-500 dark:text-ta-gray-400">
          Jika ID terdaftar, permintaan reset password Area Manager telah dikirim ke Super Admin
  ...
  return (
    <AuthLayout>
      <div className="mb-8">
        <h1 className="mb-2 text-2xl font-semibold text-ta-gray-800 dark:text-white">
          Lupa Password Area Manager
        </h1>
        <p className="text-sm text-ta-gray-500 dark:text-ta-gray-400">
          Masukkan ID Area Manager dan password baru. Super Admin akan menyetujui permintaan ini.
        </p>
      </div>
