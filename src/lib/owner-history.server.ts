  ...
  .handler(async ({ data }) => {
    ...
    if (!client)
      return {
        ...
        message: "Riwayat tidak tersedia.",
      };
    ...
    try {
      if (data.type === "sync") {
        let query = client
          ...
          .select("id,restaurant_id,stage,report_code,detail,occurred_at,resolved_at", {
            count: "exact",
          })
          .eq("stage", "sync_cache")
        ...
        if (error)
          return {
            ok: false as const,
            code: "UNAVAILABLE" as const,
            message: "Riwayat sinkronisasi tidak tersedia.",
          };
        return {
          ok: true as const,
          type: data.type,
      ...
      let query = client
        ...
        .select(
          "id,restaurant_id,audio_id,label,event_timestamp,crew_name,device_id,status,error_detail",
          { count: "exact" },
        )
      ...
      if (search.text)
        query = query.or(
          `audio_id.ilike.%${search.text}%,label.ilike.%${search.text}%,crew_name.ilike.%${search.text}%`,
        );
      const { data: rows, count, error } = await query;
      if (error)
        return {
          ok: false as const,
          code: "UNAVAILABLE" as const,
          message: "Riwayat playback tidak tersedia.",
        };
      return {
        ok: true as const,
        type: data.type,
