...
function decodeBase64url(value: string): Buffer {
  ...
  if (decoded.toString("base64url") !== value)
    throw new Error("INVALID_RESTAURANT_CODE_ENCRYPTION_KEY");
  return decoded;
}
...
export function hashRestaurantCode(code: string, baseKey: RestaurantCodeEncryptionKey): string {
  const lookupKey = deriveKey(baseKey, "table-talker/restaurant-code-lookup/v1");
...
export function encryptRestaurantCode(
  ...
  const encryptionKey = deriveKey(baseKey, "table-talker/restaurant-code-encryption/v1");
