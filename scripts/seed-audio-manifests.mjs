...
function r2Key(audioId, hash) { return `restaurants/${RESTAURANT_ID}/${audioId.replace(":", "_")}/${hash}.mp3`; }

const ANNOUNCEMENTS = {
  "seating.mp3": { id: "announcement:seating", label: "Himbauan Duduk Sesuai Nomor Meja", category: "INFO" },
  "himbauan-barang-bawaan-pelanggan.mp3": { id: "announcement:himbauan-barang-bawaan-pelanggan", label: "Himbauan Barang Bawaan Pelanggan", category: "INFO" },
  "jam-buka-resto.mp3": { id: "announcement:jam-buka-resto", label: "Informasi Jam Buka Tutup Resto", category: "INFO" },
  "outside-food.mp3": { id: "announcement:outside-food", label: "Dilarang Bawa Makanan Dari Luar", category: "LARANGAN" },
  "no-smoking.mp3": { id: "announcement:no-smoking", label: "Dilarang Merokok di Area Lobby", category: "LARANGAN" },
  "larangan-gabung-meja.mp3": { id: "announcement:larangan-gabung-meja", label: "Dilarang Gabungkan Meja", category: "LARANGAN" },
};
...
async function uploadAndRow(filePath, audioId, category, label, ordering) {
  ...
  return { restaurant_id: RESTAURANT_ID, audio_id: audioId, label, category, r2_url: url, content_hash: hash, byte_size: byteSize, active: true, ordering, catalog_version: CATALOG_VERSION };
}

async function main() {
  // 1. Delete old manifests
  console.log("Deleting old manifests...");
  const { error: delErr } = await db.from("audio_manifests").delete().eq("restaurant_id", RESTAURANT_ID).eq("catalog_version", CATALOG_VERSION);
  if (delErr) { console.error("Delete error:", delErr); process.exit(1); }

  const rows = [];

  // 2. Upload table audio
  const tableFiles = readdirSync(TABLES_DIR).filter(f => f.endsWith(".mp3")).sort((a, b) => parseInt(a) - parseInt(b));
  console.log(`Uploading ${tableFiles.length} table audio files...`);
  for (const file of tableFiles) {
    const num = parseInt(file.replace(".mp3", ""));
    const row = await uploadAndRow(join(TABLES_DIR, file), `table:${num}`, "BASE", `Meja ${num}`, num);
    rows.push(row);
  ...
  for (const file of announceFiles) {
    ...
    if (!info) { console.warn(`Unknown: ${file}`); continue; }
    const row = await uploadAndRow(join(ANNOUNCEMENTS_DIR, file), info.id, info.category, info.label, order++);
    rows.push(row);
  }
