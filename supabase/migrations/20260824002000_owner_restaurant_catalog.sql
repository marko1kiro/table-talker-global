...
returns jsonb language sql security definer set search_path = public set statement_timeout = '3000ms' as $$
  select jsonb_build_object(
    ...
    'devices', coalesce((select jsonb_agg(jsonb_build_object('id', s.id, 'display_name', s.display_name, 'device_description', s.device_description, 'audio_ready', s.audio_ready, 'connection_state', s.connection_state, 'visibility_state', s.visibility_state, 'last_seen', s.last_seen) order by s.last_seen desc) from (select * from crew_sessions where restaurant_id = r.id order by last_seen desc limit 20) s), '[]'::jsonb),
    'catalog', jsonb_build_object('total', (select count(*) from audio_manifests m where m.restaurant_id = r.id and m.catalog_version = r.catalog_version), 'items', coalesce((select jsonb_agg(jsonb_build_object('audio_id', m.audio_id, 'label', m.label, 'category', m.category, 'active', m.active, 'ordering', m.ordering) order by m.category, m.ordering) from audio_manifests m where m.restaurant_id = r.id and m.catalog_version = r.catalog_version), '[]'::jsonb)),
    'recent_playback', coalesce((select jsonb_agg(to_jsonb(p)) from (select audio_id, label, event_timestamp, crew_name, status, error_detail from playback_events where restaurant_id = r.id order by event_timestamp desc limit 20) p), '[]'::jsonb),
    'recent_errors', coalesce((select jsonb_agg(to_jsonb(e)) from (select stage, report_code, detail, occurred_at, resolved_at from operational_errors where restaurant_id = r.id order by occurred_at desc limit 20) e), '[]'::jsonb)
...
declare v_current_version integer; v_next_version integer;
begin
  if p_action not in ('upsert', 'toggle', 'delete', 'reorder') or p_audio_id = '' then raise exception 'invalid catalog mutation'; end if;
