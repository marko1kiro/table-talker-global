...
- Providers: **Email ON** (mode OTP; "Confirm email" on), **Anonymous OFF**, Phone OFF, OAuth tidak ada. Sign-up dengan password: **TETAP ON** — spike §12 poin 1 membuktikan `disable_signup` juga memblokir pembuatan user baru via OTP; kita tidak pernah memanggil `/signup` dan tidak ada form password di UI (audit call-site).
...
1. **GoTrue signup-disabled vs OTP-create-user — VERIFIED 2026-09-13: `disable_signup` MEMBLOKIR pembuatan user baru lewat email OTP. "Enable signups" harus TETAP ON.**
   Bukti sumber GoTrue (`supabase/auth` master), jalur `POST /otp`:
   - `internal/api/otp.go` — `API.Otp` hanya memanggil `shouldCreateUser(r, params)` yang memeriksa body `create_user` (default `true`); TIDAK ada pemeriksaan `config.DisableSignup` di sini.
   - `internal/api/magic_link.go` — untuk email yang belum jadi user (`isNewUser`), `API.MagicLink` membuat password acak lalu **memanggil ulang `a.Signup(fakeResponse, r)`** dengan body `SignupParams`.
   - `internal/api/signup.go` — baris pertama `Signup`: `if config.DisableSignup { return 422 signup_disabled "Signups not allowed for this instance" }`.
   ⇒ Dengan signup dimatikan: user **lama** tetap bisa login OTP/magic-link (cabang `a.Signup` dilewati), tetapi **registrasi crew (user baru) gagal total**. Tidak ada flag granular per-provider — hanya `GOTRUE_DISABLE_SIGNUP` global (`internal/conf/configuration.go`, `disable_signup split_words`).
   **Keputusan final:** provider Email ON (mode OTP) + **"Enable signups" ON**. Kita tidak pernah memanggil `/signup` dan tidak ada form password di UI (audit call-site saat implementasi). Residual risk kecil: dengan **"Confirm email" ON** respons `/signup` tidak memuat session/access token (user unconfirmed) → penyerang tidak memperoleh JWT `authenticated`, hanya memicu email konfirmasi. Kewenangan crew tetap butuh (email terverifikasi) ∧ (pairing) ∧ (device match) — §5 poin 2–3. Mitigasi spam: captcha Turnstile (Settings → CAPTCHA) + rate limit bawaan + monitor jumlah `auth.users` (poin 4 di bawah).
   **Signature terpasang (@supabase/supabase-js & @supabase/auth-js 2.112.3) — nama call yang BENAR:**
   - `auth.signInWithOtp(credentials: SignInWithPasswordlessCredentials): Promise<AuthOtpResponse>` = `{ email, options?: { shouldCreateUser?: boolean; data?; captchaToken?; emailRedirectTo? } }`; default `shouldCreateUser: true` → body `create_user: true` (`GoTrueClient.js:1857`). **`auth.requestOtp` TIDAK ADA** di versi ini — §3.2 langkah 2 harus dibaca `supabase.auth.signInWithOtp({ email })`.
   - `auth.verifyOtp(params: VerifyOtpParams): Promise<AuthResponse>`; varian `{ email: string; token: string; type: EmailOtpType }` (`types.d.ts:707-723`), `EmailOtpType = 'signup'|'invite'|'magiclink'|'recovery'|'email_change'|'email'` (`:731`). Nilai yang benar untuk kode 6 digit = **`'email'`**, bukan `'email_otp'` (di server `EmailOTPVerification = "email"`, `internal/mailer/mailer.go`) — `email_otp` hanyalah nama field pada response `admin.generateLink`.
   - Jangan kirim `shouldCreateUser: false`: untuk email yang belum punya user, GoTrue membalas 422 `otp_disabled` "Signups not allowed for otp" → membocorkan keberadaan akun (langgar §5 poin 6). Biarkan default `true`.
   - Carrier/staf: `admin.createUser(attributes: AdminUserAttributes): Promise<UserResponse>` dengan `email_confirm?: boolean`, `app_metadata?: object`, `password?` (`GoTrueAdminApi.d.ts:332`, `types.d.ts:450-482`) dan `admin.updateUserById(uid: string, attributes: AdminUserAttributes): Promise<UserResponse>` (`:577`) — **dua-duanya tersedia dan tidak dicegah `disable_signup`** (`adminUserCreate` di `internal/api/admin.go` tidak membaca `config.DisableSignup`).
   Evidence: https://supabase.com/docs/guides/auth/auth-email-passwordless · https://github.com/supabase/auth/blob/master/internal/api/otp.go · https://github.com/supabase/auth/blob/master/internal/api/magic_link.go · https://github.com/supabase/auth/blob/master/internal/api/signup.go · https://github.com/supabase/auth/blob/master/internal/api/admin.go
2. **Carrier manager/AM — VERIFIED 2026-09-13: keputusan TETAP (rotasi password + `signInWithPassword`); §3.3 bullet 2 (`generateLink`→`verifyOtp(token_hash)`) dianggap DIGANTIKAN oleh poin ini — pola itu terbukti didukung, tapi tidak dipilih.**
   - `auth.signInWithPassword(credentials: SignInWithPasswordCredentials): Promise<AuthTokenResponsePassword>` ada di 2.112.3 (`GoTrueClient.d.ts:589`) → POST `/token?grant_type=password`, mengembalikan session penuh di body (implicit flow, bukan redirect).
   - Alternatif yang SAH secara teknis: `admin.generateLink({ type:'magiclink', email })` → `data.properties.hashed_token` (`GenerateLinkProperties` = `{ action_link, email_otp, hashed_token, redirect_to, verification_type }`, `types.d.ts` ~:1009) lalu browser `verifyOtp({ token_hash, type:'magiclink' })` — varian `VerifyTokenHashParams { token_hash: string; type: EmailOtpType }` ada (`:724-729`); di server `POST /verify` cabang `magiclink` → `verifyTokenHash` (lookup `RecoveryToken`) → `recoverVerify` → terbit refresh token (`internal/api/verify.go`). Batuan wajib: dengan `token_hash` body **hanya** `token_hash` + `type` — menyertakan `email`/`redirect_to` → 400 "Only the token_hash and type should be provided".
   - Kenapa tetap keputusan lama: (a) keduanya sama-sama mengirim kredensial lewat response server fn over HTTPS — `token_hash` tidak lebih aman secara material, hanya beda umur; (b) `generateLink` **menimpa `recovery_token`** user, jadi dua permintaan paralel saling membatalkan dan kegagalan jadi sulit didebug; (c) rotasi password memberi efek yang SELARAS dengan D3 (1 email = 1 perangkat): `adminUserUpdate` → `user.UpdatePassword` → `Logout(tx, u.ID)` dengan komentar "log out user from all sessions to ensure reauthentication after password change" (`internal/models/user.go`) → sesi carrier perangkat lama otomatis mati; (d) tidak ada mekanisme handoff JWT server→browser yang lebih didukung — token akses tidak bisa ditandatangani di luar GoTrue tanpa mengambil `JWT secret` ke app server (dilarang).
   - Aturan implementasi: password baru = `crypto.randomBytes(32)` → **hex 64 char** (aman bcrypt: batas 72 byte) dan harus lolos `checkPasswordStrength` proyek (diberlakukan juga di jalur admin create/update); urutan eksak = `updateUserById({ password })` **lalu** `signInWithPassword` (sesi baru terbit setelah rotasi sehingga tidak ikut terevoke); server fn mengembalikan `{email, password}` hanya ke browser yang sama (HTTPS, tanpa logging), UI tidak pernah menyimpan password di storage; efek samping yang diterima: reload halaman setelah perangkat lain login → sesi carrier lama mati → user login ulang lewat alur manager Poin 2.
   Evidence: https://supabase.com/docs/reference/javascript/auth-signinwithpassword · https://supabase.com/docs/reference/javascript/auth-admin-generatelink · https://supabase.com/docs/reference/javascript/auth-verifyotp · https://github.com/supabase/auth/blob/master/internal/api/verify.go · https://github.com/supabase/auth/blob/master/internal/api/admin.go · https://github.com/supabase/auth/blob/master/internal/models/user.go
...
5. **Persistensi localStorage di tablet shared**: tab yang sama antar crew bergantian → setelah check-in, logout harus membersihkan sesi auth? Keputusan: role session logout TIDAK menghapus login email (sekali login = perangkat login); crew pinjam-meminjam = satu akun yang bergerak sesuai D3. Sesuai intent pemilik. **RESOLUSI implementasi:** keputusan itu hanya aman kalau ada jalan keluar — kontrol **"Keluar akun"** (`crewSignOut()` → reset total state → langkah email) TERPASANG di layar kick **dan** layar checkin `CrewLoginFlow` (affordance "Bukan <nama>? Keluar"); logout halaman role tetap tidak menyentuh sesi auth.
