...
- Risiko §12 #5 (logout & auth persist): keputusan final — **logout role-session TIDAK menghapus sesi Supabase email**; "Keluar akun" (hapus auth) **TERPASANG** di layar kick + layar checkin `CrewLoginFlow` (bukan "profile crew", yang tidak ada di Poin 3): `crewSignOut()` lalu reset semua state in-memory kembali ke langkah email, supaya tablet shared tidak menyapa crew berikutnya dengan nama crew sebelumnya.
